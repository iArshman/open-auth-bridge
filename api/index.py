...
async def handle_home(environ):
    ...
    if not is_ready:
        ...
        <div class="card" style="border-color:var(--error); background: rgba(239, 68, 68, 0.05);">
            <h3 style="color:var(--error); margin-top:0; text-align:center;">Missing Environment Variables</h3>
            <p style="color:var(--text-dim); font-size:14px; text-align:center;">Ensure GOOGLE_CLIENT_ID, GOOGLE_CLIENT_SECRET, MS_CLIENT_ID, MS_CLIENT_SECRET, and REDIRECT_URI are set.</p>
        </div>
        """
    ...
    google_keys = list(SCOPES.keys())
    ms_keys = list(MS_SCOPES.keys())
  ...
  "p": "microsoft"
            ...
            <div class="guide-step">
                ...
                <div class="step-content">
                    ...
                    <p>Convert the JSON object to a string, Base64 encode it, and pass it via the <code>state</code> parameter:</p>
                    <div class="code-container">
                        <pre>GET {scheme}://{host}/start-auth/onedrive?state=BASE64_ENCODED_STATE</pre>
                    </div>
                </div>
            </div>
            ...
            <div class="guide-step">
                ...
                <div class="step-content">
                    ...
                    <p>Once the user grants permission, Auth Bridge will POST the following payload to your webhook:</p>
                    <div class="code-container">
                        <pre>POST https://api.yourdomain.com/v1/save-tokens
    ...
    "expires_at": 1704100000.5
            ...
            <div class="guide-step" style="margin-bottom:0;">
                ...
                <div class="step-content">
                    ...
                    <p>When the access token expires, send a POST request from your backend to renew it:</p>
                    <div class="code-container">
                        <pre>POST {scheme}://{host}/refresh
Content-Type: application/json
...
{{
  ...
  "provider": "microsoft"
    ...
    <script>
        const scopes = {{
            google: {json.dumps(google_keys)},
            microsoft: {json.dumps(ms_keys)}
        }};
        ...
        function startAuth() {{
            ...
            const provider = document.getElementById('provider').value;
            const stateObj = {{ u: uid, r: 'manual', p: provider }};
...
async def handle_start_auth(environ):
    qs = environ.get("QUERY_STRING", "")
    params = parse_qs(qs)
    state_raw = params.get("state", [None])[0]
    # service is passed via the path segment stored in environ
    service = environ.get("PATH_SERVICE", "")

    if not state_raw:
        return 400, [("Content-Type", "text/plain")], b"Missing state parameter."
    ...
    if provider == "google":
        if service not in SCOPES:
            return 400, [("Content-Type", "text/plain")], f"Unknown Google service '{service}'".encode()
        ...
        if service not in MS_SCOPES:
            return 400, [("Content-Type", "text/plain")], f"Unknown Microsoft service '{service}'".encode()
        ...
        return 400, [("Content-Type", "text/plain")], b"Invalid provider specified in state."

    location = f"{auth_url}?{urlencode(auth_params)}"
    return 302, [("Location", location)], b""


async def handle_callback(environ):
    qs = environ.get("QUERY_STRING", "")
    params = parse_qs(qs)
    ...
    if not code or not state_str:
        return 400, [("Content-Type", "text/plain")], b"Auth flow interrupted. Missing code or state."
    ...
    if provider == "google":
        ...
        return 400, [("Content-Type", "text/plain")], b"Unknown provider in callback state."
        ...
        async with ClientSession() as session:
            async with session.post(token_url, data=token_payload) as resp:
                ...
                if resp.status != 200:
                    ...
                    return 400, [("Content-Type", "text/plain")], f"OAuth Provider Error: {err}".encode()
...
<body style="display:flex; align-items:center; justify-content:center; min-height:100vh; padding:20px;">
    <div class="card" style="max-width:700px; width:100%;">
        ...
        <div class="code-container">
            <button class="copy-btn" onclick="copyCode()">Copy JSON</button>
        ...
        async with ClientSession() as session:
            async with session.post(return_url, json=payload, timeout=15) as bot_resp:
                if bot_resp.status == 200:
                    body = f"<html><body style='background:#0a0b10; color:#ffffff; font-family:sans-serif; text-align:center; padding-top:100px;'><h1>Tokens Delivered</h1><p>Webhook returned 200 OK. You can close this window.</p></body></html>"
                    return 200, [("Content-Type", "text/html; charset=utf-8")], body.encode("utf-8")
                err = await bot_resp.text()
                return 500, [("Content-Type", "text/plain")], f"Webhook Failed: {bot_resp.status}. {err}".encode()

    except Exception as e:
        logger.error(f"Callback error: {e}", exc_info=True)
        return 500, [("Content-Type", "text/plain")], f"Internal Server Error: {str(e)}".encode()


async def handle_refresh(body_bytes):
    try:
        ...
        if not rt:
            return 400, [("Content-Type", "application/json")], json.dumps({"error": "Missing refresh_token"}).encode()
        ...
        if provider == "google":
            ...
            return 400, [("Content-Type", "application/json")], json.dumps({"error": "Invalid provider"}).encode()

        async with ClientSession() as session:
            async with session.post(token_url, data=token_payload) as resp:
                res = await resp.json()
                if resp.status != 200:
                    return 400, [("Content-Type", "application/json")], json.dumps({"error": "Failed to refresh", "details": res}).encode()
                result = {
...
async def handle_terms(environ):
    scheme = environ.get("wsgi.url_scheme", "https")
    html = f"""<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
    <div class="container">
        <nav class="nav">
            <a href="/" class="logo"><div class="logo-box">UA</div>Open Auth Bridge</a>
            <div class="nav-links"><a href="/terms">Terms</a><a href="/privacy">Privacy</a></div>
        </nav>
        ...
        <div class="footer">Open Auth Bridge &copy; 2026 &middot; <a href="https://arshman.me">arshman.me</a></div>
    </div>
</body>
</html>"""
    return 200, [("Content-Type", "text/html; charset=utf-8")], html.encode("utf-8")
...
async def handle_privacy(environ):
    html = f"""<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
...
<body>
    <div class="container">
        ...
        <div class="card legal-content" style="padding: 40px 48px;">
            ...
            <p class="updated">Last updated: May 2026</p>
            <div class="highlight-box"><strong>Summary:</strong> We act only as a delivery bridge for your Google and Microsoft tokens. We do not store, sell, or analyze your personal data.</div>
            ...
            <ul>
                ...
                <li><strong>Routing Metadata:</strong> The <code>state</code> parameter provided by the initiating application to ensure tokens are routed to the correct destination webhook.</li>
            </ul>
            <h2>2. How We Use Your Information</h2>
            <p>Your data is used exclusively to complete the standard OAuth 2.0 flow — to deliver tokens to the initiating application and to refresh expired tokens when requested. We do not use your data for tracking, analytics, advertising, or profiling.</p>
            <h2>3. Data Storage &amp; Retention</h2>
            <p><strong>We do not maintain a database of your tokens.</strong> Tokens are held in-memory exclusively during the callback processing and webhook delivery phase. Once the HTTP response is sent, tokens are immediately discarded from server memory.</p>
            ...
            <p>Because we do not store your tokens, you cannot delete your account with us (as you do not have one). You retain full control and can revoke access at any time directly through your provider:</p>
            <ul>
...
class handler(BaseHTTPRequestHandler):
    def do_GET(self):
        ...
        # Route: /start-auth/{service}
        if path.startswith("/start-auth/"):
            service = path[len("/start-auth/"):]
            environ["PATH_SERVICE"] = service
            status, headers, body = asyncio.run(handle_start_auth(environ))
        elif path == "/oauth_callback":
            status, headers, body = asyncio.run(handle_callback(environ))
        elif path == "/terms":
            status, headers, body = asyncio.run(handle_terms(environ))
        elif path == "/privacy":
            status, headers, body = asyncio.run(handle_privacy(environ))
        elif path == "/":
            status, headers, body = asyncio.run(handle_home(environ))
        else:
    ...
    def do_POST(self):
        ...
        if path == "/refresh":
            status, headers, body = asyncio.run(handle_refresh(body_bytes))
        else:
            status, headers, body = 404, [("Content-Type", "text/plain")], b"Not Found"
